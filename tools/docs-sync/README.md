...
After bootstrap, every committed synchronization output—accepted baseline, fingerprint, change report, optional runtime report, generated interface manual, and accepted screenshots—may change only in a `docs-sync/<full-source-sha>-<run-id>-<publication-commit-sha>` pull request created by `github-actions[bot]` in this repository. The full publication commit SHA embedded in the branch name must equal the pull request's exact head, and the full source SHA must equal its exact accepted base, so neither replacement commits nor a stale base can inherit approval. The embedded source SHA and run ID must resolve through GitHub's Actions API to the trusted `docs-interface-sync` workflow running for that exact push to this repository's `main`. That exact run must also contain its immutable provenance artifact binding repository, trusted workflow path, source/base SHA, run ID, PR number, branch, and publication commit; another write-enabled workflow cannot borrow either the bot identity or an old legitimate run ID. A separate `pull_request_target` workflow is evaluated from the base branch, checks out and executes only accepted-base guard code, and treats the fetched pull-request head solely as diff data. Successful ownership status is available only for pull requests whose authoritative base is this repository's `main` branch, and `edited` events revalidate any base change. Every `main` advance immediately posts a pending ownership status on open generated publications and dispatches both base ownership and deterministic artifact regeneration against the new base; a stale immutable publication fails and must be replaced. The triggering revision therefore cannot weaken or remove its own ownership check; modification or deletion of any synchronized publication artifact from every other branch, repository, or author is rejected. Manual workflow dispatch without a publication PR is restricted to `main` and compares against the baseline at the dispatched `github.sha`.

GitHub suppresses ordinary workflow events caused by `GITHUB_TOKEN`, so the publication job explicitly uses the supported `workflow_dispatch` exception after creating a bot PR: it dispatches base-controlled ownership verification on `main` and source verification on the generated branch. The ownership workflow resolves pull-request and trusted-run metadata through GitHub's API and publishes a `docs-baseline-ownership` commit status on the exact PR head. Generated-branch source verification resolves the authoritative PR base and embedded source revision, regenerates deterministic candidate outputs against that base, and fails on tracked, deleted, or untracked artifact drift.
...
Every entry point validates the screen manifest before capture, comparison or publication. Screen IDs must be unique, and screenshot targets must be unique canonical repository-relative paths under `docs/screenshots/current/`, preventing aliases or one screen's failure/removal record from deleting another screen's accepted image.
