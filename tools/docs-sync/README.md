...
After bootstrap, `current-baseline.json` may change only in a `docs-sync/<source-sha>-<run-id>` pull request created by `github-actions[bot]`. The pull-request job materializes the guard implementation from the accepted base commit before executing it, so the triggering revision cannot weaken the guard; direct baseline edits from every other branch or author are rejected. Manual workflow dispatch compares against the baseline at the dispatched `github.sha`.
