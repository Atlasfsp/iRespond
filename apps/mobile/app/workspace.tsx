...
export default function Workspace() {
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); void load(); }} />}>
    ...
    {items.map((item) => <Pressable key={item.title} style={[styles.card, item.tone === 'safety' && styles.safetyCard]} onPress={() => router.push(item.route as never)} accessibilityRole="button"><View style={{ flex: 1 }}><Text style={[styles.cardTitle, item.tone === 'safety' && styles.safetyTitle]}>{item.title}</Text><Text style={styles.muted}>{item.description}</Text></View><Text style={styles.chevron}>›</Text></Pressable>)}
