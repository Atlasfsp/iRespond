...
export default function ImpactFeed() {
  ...
  useEffect(() => {
    ...
    async function sync() {
      ...
      const count = await pendingNeedCount();
      if (active) setPendingSync(count);
    }
    void sync();
    return () => { active = false; };
  }, []);
