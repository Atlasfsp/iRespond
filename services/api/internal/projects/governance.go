...
var (
	ErrForbiddenProjectAction = errors.New("project action not permitted")
...
func (m *Manager) HasAnyProjectRole(ctx context.Context, projectID, actorID string, roles ...string) (bool,error) {
	...
	var ok bool
	err:=m.pool.QueryRow(ctx,`SELECT EXISTS(SELECT 1 FROM project_roles WHERE project_id=$1 AND actor_id=$2 AND role=ANY($3))`,projectID,actorID,roles).Scan(&ok)
	return ok,err
}

func (m *Manager) InviteRole(ctx context.Context,id,projectID,actorID,role,invitedBy string)(RoleInvite,error){
	if !validProjectRole(role){return RoleInvite{},fmt.Errorf("invalid project role")}
	if _,err:=m.Get(ctx,projectID);err!=nil{return RoleInvite{},err}
	var v RoleInvite
	err:=m.pool.QueryRow(ctx,`INSERT INTO project_role_invites(id,project_id,invited_actor_id,role,invited_by) VALUES($1,$2,$3,$4,$5) RETURNING id,project_id,invited_actor_id,role,invited_by,status,created_at,responded_at`,id,projectID,actorID,role,invitedBy).Scan(&v.ID,&v.ProjectID,&v.InvitedActorID,&v.Role,&v.InvitedBy,&v.Status,&v.CreatedAt,&v.RespondedAt)
	return v,err
}

func (m *Manager) AcceptRole(ctx context.Context,inviteID,actorID string)(RoleInvite,error){
	tx,err:=m.pool.Begin(ctx);if err!=nil{return RoleInvite{},err};defer tx.Rollback(ctx)
	var v RoleInvite
	...
	if errors.Is(err,pgx.ErrNoRows){return RoleInvite{},ErrRoleInviteNotFound};if err!=nil{return RoleInvite{},err}
	if _,err=tx.Exec(ctx,`INSERT INTO project_roles(project_id,actor_id,role) VALUES($1,$2,$3) ON CONFLICT DO NOTHING`,v.ProjectID,actorID,v.Role);err!=nil{return RoleInvite{},err}
	if v.Role=="project_manager"{if _,err=tx.Exec(ctx,`UPDATE action_projects SET project_manager_id=$2,updated_at=now() WHERE id=$1`,v.ProjectID,actorID);err!=nil{return RoleInvite{},err}}
	if err:=tx.Commit(ctx);err!=nil{return RoleInvite{},err};return v,nil
}
...
func (m *Manager) AddMilestone(ctx context.Context,id,projectID,title,description string,sequence int,targetAt *time.Time)(Milestone,error){
	if strings.TrimSpace(title)==""||sequence<=0{return Milestone{},fmt.Errorf("title and positive sequence are required")}
	var v Milestone
	...
	if err!=nil&&strings.Contains(strings.ToLower(err.Error()),"foreign key"){return Milestone{},ErrProjectNotFound}
	return v,err
}
...
func (m *Manager) TransitionMilestone(ctx context.Context,projectID,milestoneID,target,actorID string,canValidate bool)(Milestone,error){
	tx,err:=m.pool.Begin(ctx);if err!=nil{return Milestone{},err};defer tx.Rollback(ctx)
	var v Milestone
	...
	if errors.Is(err,pgx.ErrNoRows){return Milestone{},ErrMilestoneNotFound};if err!=nil{return Milestone{},err}
	if !allowedMilestoneTransition(v.Status,target)||(target=="validated"&&!canValidate){return Milestone{},ErrInvalidMilestoneTransition}
	completed:=v.CompletedAt;if target=="validated"{now:=time.Now().UTC();completed=&now}
	...
	if _,err=tx.Exec(ctx,`INSERT INTO milestone_status_history(milestone_id,project_id,from_status,to_status,actor_id) VALUES($1,$2,$3,$4,$5)`,milestoneID,projectID,previousMilestoneStatus(target,v.Status),target,actorID);err!=nil{return Milestone{},err}
	if err:=tx.Commit(ctx);err!=nil{return Milestone{},err};return v,nil
}
...
func (m *Manager) TransitionProject(ctx context.Context,projectID,target,actorID string)(Project,error){
	...
	if errors.Is(err,pgx.ErrNoRows){return Project{},ErrProjectNotFound};if err!=nil{return Project{},err}
	from:=p.Status;if !allowedProjectTransition(from,target){return Project{},ErrInvalidProjectTransition}
	if err:=projectReadiness(ctx,tx,projectID,target,p.ProjectManagerID);err!=nil{return Project{},err}
...
func projectReadiness(ctx context.Context,tx pgx.Tx,projectID,target string,managerID *string)error{
	var count int
	switch target{
	case "approved":
		if managerID==nil||*managerID==""{return fmt.Errorf("project manager must accept role before approval")}
		if err:=tx.QueryRow(ctx,`SELECT count(*) FROM project_milestones WHERE project_id=$1 AND status<>'cancelled'`,projectID).Scan(&count);err!=nil{return err};if count==0{return fmt.Errorf("at least one milestone is required before approval")}
	case "validating":
		if err:=tx.QueryRow(ctx,`SELECT count(*) FROM project_milestones WHERE project_id=$1 AND status NOT IN ('submitted','validated','cancelled')`,projectID).Scan(&count);err!=nil{return err};if count>0{return fmt.Errorf("all active milestones must be submitted before validation")}
	case "maintaining":
		if err:=tx.QueryRow(ctx,`SELECT count(*) FROM project_milestones WHERE project_id=$1 AND status NOT IN ('validated','cancelled')`,projectID).Scan(&count);err!=nil{return err};if count>0{return fmt.Errorf("all active milestones must be validated before maintenance")}
	case "completed":
		if err:=tx.QueryRow(ctx,`SELECT count(*) FROM project_roles WHERE project_id=$1 AND role='maintenance_owner'`,projectID).Scan(&count);err!=nil{return err};if count==0{return fmt.Errorf("maintenance owner is required before completion")}
	}
	return nil
}
...
func previousMilestoneStatus(target,current string)string{return current}
