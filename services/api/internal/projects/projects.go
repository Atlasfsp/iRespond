...
func New(ctx context.Context, databaseURL string) (*Manager,error) {
	pool,err:=pgxpool.New(ctx,databaseURL);if err!=nil{return nil,err}
...
func(m *Manager)Convert(ctx context.Context,id,needID,title,description,communityID,actorID string)(Project,error){
	...
	_,err=tx.Exec(ctx,`INSERT INTO project_roles(project_id,actor_id,role) VALUES($1,$2,'community_steward')`,id,actorID);if err!=nil{return Project{},err}
	_,err=tx.Exec(ctx,`INSERT INTO outbox_events(id,aggregate_type,aggregate_id,event_type,payload) VALUES($1,'action_project',$2,'project.created',jsonb_build_object('projectId',$2,'sourceNeedId',$3,'createdBy',$4))`,id+"-created",id,needID,actorID);if err!=nil{return Project{},err}
...
func(m *Manager)Get(ctx context.Context,id string)(Detail,error){
	...
	milestones:=[]Milestone{};rows,err:=m.pool.Query(ctx,`SELECT id,project_id,title,description,status,sequence,target_at,completed_at FROM project_milestones WHERE project_id=$1 ORDER BY sequence`,id);if err!=nil{return Detail{},err};for rows.Next(){var v Milestone;if err:=rows.Scan(&v.ID,&v.ProjectID,&v.Title,&v.Description,&v.Status,&v.Sequence,&v.TargetAt,&v.CompletedAt);err!=nil{rows.Close();return Detail{},err};milestones=append(milestones,v)};rows.Close()
	needs:=[]ContributionNeed{};rows,err=m.pool.Query(ctx,`SELECT id,project_id,kind,description,quantity_note,status FROM contribution_needs WHERE project_id=$1 ORDER BY created_at`,id);if err!=nil{return Detail{},err};for rows.Next(){var v ContributionNeed;if err:=rows.Scan(&v.ID,&v.ProjectID,&v.Kind,&v.Description,&v.QuantityNote,&v.Status);err!=nil{rows.Close();return Detail{},err};needs=append(needs,v)};rows.Close()
