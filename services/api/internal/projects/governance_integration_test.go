...
func TestGovernedProjectLifecycle(t *testing.T){
	...
	if _,err:=pool.Exec(ctx,`INSERT INTO needs(id,title,description,category,reporter_id,verification_state,sdg_tags,latitude,longitude) VALUES('need-gov','Repair school roof','x','education','reporter','community_confirmed','{4,11}',6.53,3.38)`);err!=nil{t.Fatal(err)}
