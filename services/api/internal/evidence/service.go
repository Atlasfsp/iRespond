...
func (s *Service) Initiate(ctx context.Context, needID, uploaderID, contentType string, sizeBytes int64, sha256Hex string) (InitiatedUpload, error) {
	...
	objectKey := path.Join("needs",needID,"evidence",id+ext)
...
func (s *Service) Complete(ctx context.Context, needID, evidenceID, uploaderID string) (Record,error) {
	var objectKey, contentType, owner string
	var sizeBytes int64
	var sha string
	...
	if errors.Is(err,pgx.ErrNoRows){return Record{},ErrEvidenceNotFound};if err!=nil{return Record{},err}
	if owner!=uploaderID{return Record{},ErrEvidenceNotFound}
	info,err:=s.objects.StatObject(ctx,s.bucket,objectKey,minio.StatObjectOptions{})
	if err!=nil{return Record{},err}
	if info.Size!=sizeBytes{return Record{},ErrObjectMismatch}
	var rec Record
	err=s.db.QueryRow(ctx,`UPDATE need_evidence SET status='available',available_at=now() WHERE id=$1 AND status='pending_upload' RETURNING id,need_id,content_type,size_bytes,COALESCE(sha256_hex,''),status,created_at,available_at`,evidenceID).Scan(&rec.ID,&rec.NeedID,&rec.ContentType,&rec.SizeBytes,&rec.SHA256Hex,&rec.Status,&rec.CreatedAt,&rec.AvailableAt)
	if errors.Is(err,pgx.ErrNoRows){return Record{},ErrEvidenceNotFound}
	return rec,err
}
...
func (s *Service) AccessURL(ctx context.Context, needID, evidenceID string)(string,error){
	...
	u,err:=s.objects.PresignedGetObject(ctx,s.bucket,objectKey,5*time.Minute,url.Values{})
	if err!=nil{return "",err};return u.String(),nil
}

func allowedContentType(v string) bool {
	switch v { case "image/jpeg","image/png","image/webp","video/mp4","video/quicktime": return true; default:return false }
}
