...
func gatewayConfigFromEnvironment() gatewayConfig {
	cfg := gatewayConfig{MaxBodyBytes: 1 << 20, RequestsPerMinute: 240}
	if value, err := strconv.ParseInt(strings.TrimSpace(os.Getenv("MAX_REQUEST_BODY_BYTES")), 10, 64); err == nil && value > 0 {
		cfg.MaxBodyBytes = value
	}
	if value, err := strconv.Atoi(strings.TrimSpace(os.Getenv("REQUESTS_PER_MINUTE"))); err == nil && value > 0 {
...
func newClientRateLimiter(limit int) *clientRateLimiter {
	return &clientRateLimiter{limit: limit, clients: make(map[string]windowCounter)}
}
...
func (l *clientRateLimiter) allow(client string, now time.Time) (bool, int) {
	...
	if window.Reset.IsZero() || !now.Before(window.Reset) {
		window = windowCounter{Reset: now.Add(time.Minute)}
	}
	if window.Count >= l.limit {
		return false, max(1, int(time.Until(window.Reset).Seconds()))
	}
	window.Count++
...
func newRequestMetrics() *requestMetrics {
	return &requestMetrics{values: make(map[string]requestMetric)}
}
...
func (m *requestMetrics) observe(method, route string, status int, duration time.Duration) {
	...
	key := method + "\x00" + route + "\x00" + class
	m.mu.Lock()
	metric := m.values[key]
	metric.Method, metric.Route, metric.StatusClass = method, route, class
	metric.Count++
	metric.DurationSeconds += duration.Seconds()
	m.values[key] = metric
	m.mu.Unlock()
}
...
func (m *requestMetrics) writePrometheus(w http.ResponseWriter) {
	m.mu.Lock()
	values := make([]requestMetric, 0, len(m.values))
...
func gatewayMiddleware(next http.Handler) http.Handler {
	...
	limiter := newClientRateLimiter(cfg.RequestsPerMinute)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		started := time.Now()
		capture := &responseCapture{ResponseWriter: w}
		requestID := validCorrelationID(r.Header.Get("X-Request-ID"))
		if requestID == "" {
			requestID = newID()
		}
		...
		if r.ContentLength > cfg.MaxBodyBytes && r.ContentLength >= 0 {
			writeJSON(capture, http.StatusRequestEntityTooLarge, map[string]string{"error": "request body exceeds configured limit"})
			gatewayRequestMetrics.observe(r.Method, "gateway/body-limit", capture.status, time.Since(started))
			return
		}
		if r.Body != nil {
			r.Body = http.MaxBytesReader(capture, r.Body, cfg.MaxBodyBytes)
		}
		if !gatewayExemptPath(r.URL.Path) {
			allowed, retryAfter := limiter.allow(remoteClient(r.RemoteAddr), time.Now())
			if !allowed {
				capture.Header().Set("Retry-After", strconv.Itoa(retryAfter))
				writeJSON(capture, http.StatusTooManyRequests, map[string]string{"error": "request rate limit exceeded"})
				gatewayRequestMetrics.observe(r.Method, "gateway/rate-limit", capture.status, time.Since(started))
				return
			}
		}
		...
		gatewayRequestMetrics.observe(r.Method, r.Pattern, capture.status, time.Since(started))
	})
}

func gatewayExemptPath(path string) bool {
	switch path {
	case "/livez", "/readyz", "/healthz", "/metrics", "/version":
		return true
	default:
		return false
	}
...
func remoteClient(remoteAddr string) string {
...
func traceIDFromTraceparent(value string) string {
	...
	if len(parts) != 4 || len(parts[0]) != 2 || len(parts[1]) != 32 || len(parts[2]) != 16 || len(parts[3]) != 2 || strings.EqualFold(parts[0], "ff") || allZero(parts[1]) || allZero(parts[2]) {
		return ""
	}
