...
func registerFundingRoutes(mux *http.ServeMux,identity auth.Verifier)func(){
	...
	available:=func(w http.ResponseWriter)bool{if svc==nil{writeJSON(w,http.StatusServiceUnavailable,map[string]string{"error":"funding service is not configured"});return false};return true}
	canManage:=func(r *http.Request,p auth.Principal)(bool,error){if p.HasRole("project_steward")||p.HasRole("trust_safety_admin"){return true,nil};return svc.CanManage(r.Context(),r.PathValue("id"),p.Subject)}
